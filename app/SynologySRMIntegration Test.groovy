...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Synology SRM Router Integration", install: true, uninstall: true) {
        ...
        section("Diagnostics") {
            paragraph "Forces a fresh login attempt and logs a detailed request/response report — useful when filing a bug report (e.g. against a Hubitat platform/firmware issue). Check Logs after tapping."
            input "runDiagnosticsBtn", "button", title: "Run Diagnostics Now"
        }
        section {
            paragraph "<small>Synology SRM Integration v${VERSION} &nbsp;•&nbsp; Hub firmware: ${hubFirmwareVersion()}</small>"
        }
    }
}
...
def appButtonHandler(String btn) {
    if (btn == "runDiagnosticsBtn") runDiagnostics()
}
...
private boolean login() {
    ...
    // Credentials go in a form-urlencoded POST body, NOT the URL query string. A query string
    // requires client-side percent-encoding, and a change in that encoding between HTTP client
    // library versions (e.g. an Apache HttpClient 4.x -> 5.x bump for HTTP/2 + TLS 1.3, as seen
    // on a Hubitat beta firmware) can silently mangle the password while the request stays
    // syntactically valid — the router then honestly (and misleadingly) reports "wrong password"
    // even though the real credentials are correct. A form body follows a different code path and
    // sidesteps this whole class of bug; it's also better practice regardless (keeps the password
    // out of any URL that might be logged along the way).
    ...
    // Never log the password itself — just its length, to rule out truncation/encoding bugs
    // without exposing the secret.
    def maskedBody = body.collectEntries { k, v ->
        [(k): (k == "passwd" ? "<${v?.toString()?.length() ?: 0} chars>" : v)]
    }
    log.info "[LOGIN] Request -> POST ${params.uri}${params.path}  body=${maskedBody}  " +
             "hubFirmware=${hubFirmwareVersion()}  appVersion=${VERSION}"

    try {
        httpPost(params) { resp ->
            log.info "[LOGIN] Response <- httpStatus=${safe { resp.status }} " +
                      "contentType=${safe { resp.contentType }} headers=${safe { resp.headers }}"

            // IMPORTANT: resp.data behaves like a one-shot stream on this platform — reading it
            // twice (once for the raw-body log, once to parse) silently corrupts the second read.
            // Read it exactly once here, then parse that same captured text.
            String raw = safe { rawText(resp) }
            logDebug "[LOGIN] Raw body: ${raw}"
            def data = parseRaw(raw)

            if (data?.success && data?.data?.sid) {
                state.sid = data.data.sid
                logDebug "[LOGIN] OK — sid acquired (${state.sid?.toString()?.length()} chars)"
            } else {
                def code = data?.error?.code
                log.error "[LOGIN] FAILED — httpStatus=${safe { resp.status }} success=${data?.success} " +
                          "errorCode=${code} (${authErrorText(code)}) fullResponse=${data}"
            }
        }
    } catch (e) {
        log.error "[LOGIN] EXCEPTION — ${e.class?.name}: ${e.message}"
        try {
            // groovyx.net.http exceptions (e.g. HttpResponseException) carry the actual response —
            // surface it since Hubitat's own message is often just "status code: XXX".
            def r = e.hasProperty('response') ? e.response : null
            if (r != null) log.error "[LOGIN] EXCEPTION response — status=${safe { r.status }} body=${safe { r.data }}"
        } catch (ignored) { }
    }
    return state.sid != null
}

// Synology SYNO.API.Auth error codes (per the official DSM/SRM Login Web API guide).
private String authErrorText(code) {
    switch (code) {
        case null: return "no error code in response — likely a parse failure or empty body; check the raw body log line above"
        ...
        case 104: return "the requested version does not support this method"
        case 105: return "insufficient user privilege"
        ...
        case 403: return "2-factor authentication code required"
        case 404: return "failed to authenticate 2-factor authentication code"
        case 406: return "must authenticate with 2-factor authentication code"
        case 407: return "blocked IP source (too many failed logins?)"
        case 408: return "expired password, cannot change"
        case 409: return "expired password"
        case 410: return "password must be changed"
        default:  return "unrecognized error code — see Synology API docs"
    }
}

// Best-effort hub firmware version, for including in bug reports. Wrapped because the property
// itself may differ or be unavailable across Hubitat platform versions.
private String hubFirmwareVersion() {
    return safe { location?.hubs?.getAt(0)?.firmwareVersionString ?: location?.hub?.firmwareVersionString }
}

// Evaluate a closure, returning its result as a string or a "N/A (reason)" placeholder on any
// failure — so one bad diagnostic field never aborts the rest of the log line.
private String safe(Closure c) {
    try { return c()?.toString() ?: "null" } catch (e) { return "N/A (${e.class?.simpleName}: ${e.message})" }
}

// Extracts the raw response body text regardless of whether Hubitat already parsed it to a Map.
// Only ever call this ONCE per response — resp.data behaves like a one-shot stream on this
// platform, so a second read (e.g. via asMap()) silently returns garbage, not the real body.
private String rawText(resp) {
    def d = resp?.data
    if (d == null) return null
    if (d instanceof String) return d
    if (d instanceof Map) return JsonOutput.toJson(d)   // already parsed — reconstitute for logging
    try { return d.getText() } catch (ignored) { return d?.toString() }
}

// Parses already-captured raw text into a Map (does not touch resp.data — safe to call after
// rawText() has already consumed the response stream).
private Map parseRaw(String raw) {
    if (raw == null) return null
    try { return parseJson(raw) }
    catch (e) { log.error "Could not parse response JSON: ${e.message}. Raw (first 500 chars): ${raw?.take(500)}"; return null }
}

/**
 * One-click diagnostic report for bug reports (e.g. to Hubitat support about platform/firmware
 * regressions). Forces a fresh login attempt — bypassing the cached session — so the verbose
 * [LOGIN] logging above always fires, and prints a summary block around it.
 */
void runDiagnostics() {
    log.warn "===== Synology SRM Integration — Diagnostics ====="
    log.warn "App version:      ${VERSION}"
    log.warn "Hub firmware:      ${hubFirmwareVersion()}"
    log.warn "Hub type:          ${safe { location?.hubs?.getAt(0)?.type ?: location?.hub?.type }}"
    log.warn "Router:            ${settings.host}:${settings.port}  https=${settings.useHTTPS != false}"
    log.warn "Username set:      ${settings.username ? "yes (${settings.username.length()} chars)" : 'NO'}"
    log.warn "Password set:      ${settings.password ? "yes (${settings.password.length()} chars)" : 'NO'}"
    log.warn "Cached session:    ${state.sid ? 'present' : 'none'}"
    state.sid = null   // force login() to run its full request/response logging below, not short-circuit
    boolean ok = login()
    log.warn "Fresh login result: ${ok ? 'SUCCESS' : 'FAILED — see [LOGIN] lines above for detail'}"
    log.warn "===== End Diagnostics — copy everything from here up for your bug report ====="
}
